...
public class BullsAndCows {
    ...
    public static String getHint2Version2(String secret, String guess) {
        // 没想到还有更牛的
        int bulls = 0, cows = 0;
        int[] arr = new int[10];
        for (int i = 0; i < secret.length(); i ++) {
            if (secret.charAt(i) == guess.charAt(i)) {
                ...
                // 精华 但是一般真的想不到，知道要两个在一个数上面加减，没想到可以这么做
        ...
        return new StringBuffer().append(cows).append("A").append(bulls).append("B").toString();
    }
    ...
    public static String getHintSecondVersion(String secret, String guess) {
        ...
        for (int i = 0; i < length; i++) {
            ...
            Integer count = map.get(guessChar);
            if (count != null && count > 0) {
                cows++;
                count--;
                map.put(guessChar, count);
            }
        }
