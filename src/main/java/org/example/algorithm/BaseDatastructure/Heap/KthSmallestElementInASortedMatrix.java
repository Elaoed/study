...
public class KthSmallestElementInASortedMatrix {
     ...
     * 这里写一个题解里面看到的二分法
     */
    public static int findCountSmallerThanMid(int mid, int[][] matrix) {
        // 他的查找是从左下角开始看当前列有多少比mid小 直接一次性加，当然也可以从右上角看当前行
        int count = 0;
    ...
    public static int version2(int[][] matrix, int k) {
        ...
        while (left < right) {
            ...
            int count = findCountSmallerThanMid(mid, matrix);
            if (count < k) {
                left = mid + 1;
            } else {
                right = mid;
